...
2026-09-16T03:12:34Z segment-takes: begin — per-segment TAKE browsing in the Shot player: derived take universe from retained render versions, one persisted per-placement selection, one selection law at every clip resolver, fresh renders auto-select, side-by-side COMPARE, whole-shot history numerals retired in favor of a read-only RENDER HISTORY plate, duplicate segment card fixed. Plan: plans/20260916T031234Z-segment-takes.md

2026-09-16T03:44:59Z segment-takes: completed — BUILT (uncommitted, atop 4d2a916). Model: `ShotSegmentTakeSelection` on ProjectShot (tolerant, pruned in normalized()), derived take universe `shotSegmentTakes` + `shotSelectedSegmentTakeClip`, THE FRESH TAKE LAW in upsertingRenderVersion (ready-gated, clipPath freshness), snapshot/undo field, engine `shotSegmentTakeImpact`/`useShotSegmentTake` (sync, in-flight and missing-file refusals, effective-clip no-op), selection rung at every resolver (presentation, catalog, assembly, render reuse source, output fingerprint overlay, extend/ending tail anchors, combined cuts, branches, excursions), stale law rung for a re-picked rendered tail. UI: `ShotTakeOption` adapters, take strip on segment cards and Scene-row TAKES popover, player chip (segment · take · in film), side-by-side COMPARE (two muted players on one host clock), ←/→ ↩/U 1/2 Space in a Takes keymap scope, Escape rung, Home/End honor a previewed take; whole-shot numerals + HISTORY mode retired, plate renamed RENDER HISTORY with an honest whole-file preview, duplicate segment card fixed (`shotUnrepresentedWorkTiles`), segment CTA reads Render new take on every generated card incl. single-segment Shots. Verification: swift build clean (no new warnings); swift test 1317 passed (25 new: ShotSegmentTakeTests 17, ShotTakeOptionsTests 8); check_source_hygiene clean; git diff --check clean; dist/LitScenes Development.app rebuilt — RELAUNCH required. NOT done: owner smoke on nwff SHOT I (two takes, compare, ⌘Z, render a third take); no paid call made; no commit.

2026-09-17T02:21:46Z segment-reuse-and-take-readability: begin approved implementation of saved-video Shot-row paste, segment Finder reveal and readable Continuation Takes.

2026-09-17T02:42:47Z segment-reuse-and-take-readability: completed in the uncommitted public working tree. Saved-video row/thumbnail paste retains provenance and supports undo/redo; segment Finder reveal is wired; Continuation Takes has explicit light appearance, dark readable labels and scoped disabled styling. Build, all 1,317 existing tests, both source-hygiene checks and whitespace checks pass. Development app packaged and strict signature verified; packaged and built Mach-O UUIDs match. Temporary-project walkthrough verifies whole/ranged/repeated paste, reopened provenance, undo/redo and append to a rendered Shot. Light/dark native view captures reviewed. REUSE unavailable. No live project edits, provider calls, commits, pushes or private pin updates.
